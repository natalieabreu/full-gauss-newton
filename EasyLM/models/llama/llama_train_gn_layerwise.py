...
from EasyLM.layerwise_utils import (LayerState, get_layer_params, merge_layer_params, 
...
def main(argv):
    ...
    get_layer_param_keys_partial = partial(get_layer_param_keys, num_hidden_layers=llama_config.num_hidden_layers)
    get_layer_params_partial = partial(get_layer_params, num_hidden_layers=llama_config.num_hidden_layers)
    merge_layer_params_partial = partial(merge_layer_params, num_hidden_layers=llama_config.num_hidden_layers)
    ...
    def build_optimizer(lr_sched, b1, b2, grad_clip=None, wd=0.0, 
                        optimizer_type='adamw', layer=None):
        if optimizer_type == 'adamw':
            ...
            if layer is not None:
                print(f"[build_optimizer] Binding optimizer to layer: {layer}")
                
                def param_layer_selector(params):
                    layer_keys = get_layer_param_keys_partial(params, layer)
                    layer_keys_set = set(layer_keys)

                    def assign(path, v):
                        full_path = ".".join(path)
                        assignment = 'trainable' if full_path in layer_keys_set else 'frozen'
                        # print(f"[Selector for {layer}] {full_path} → {assignment}")
                        return assignment

                    return path_aware_map(assign, params)
                
                if layer in {'wte', 'lm_head', 'ln_f'}:
                    partition_optimizers = {'trainable': adamw_chain, 'frozen': optax.set_to_zero()}
                else:
                    partition_optimizers = {'trainable': muon_chain, 'frozen': optax.set_to_zero()}

                optimizer = optax.multi_transform(partition_optimizers, param_layer_selector)
            else:
                transform_dict = {
                    'adamw': adamw_chain,
                    'muon':   muon_chain,
    ...
    for layer in layers: # initialize optimizers for each layer
        # inner_lr = FLAGS.inner_loop_lr / 10 if layer in ['wte', 'lm_head'] else FLAGS.inner_loop_lr
        ...
        tayl_solver = build_optimizer(lr_sched, FLAGS.inner_b1, FLAGS.inner_b2, FLAGS.inner_clip_gradient, 
                                      FLAGS.optimizer_wd, FLAGS.optimizer_type, layer=layer)
        all_schedules.append(lr_sched)
        all_optimizers.append(tayl_solver)

    def make_layer_state_creator(layer_idx, layer_name):
        tx = all_optimizers[layer_idx]          # capture the correct optimizer once
        def creator(params):
            return LayerState.create(
                full_params=params,
                tx=tx,
                apply_fn=model.apply,           # already static in LayerState
                layer=layer_name,               # captured constant, no tracing
                num_hidden_layers=llama_config.num_hidden_layers,
            )
        return creator
            

    def create_layer_state_from_params(params, layer_idx, layer_name):
        return LayerState.create(full_params=params, tx=all_optimizers[layer_idx], apply_fn=model.apply, layer=layer_name, num_hidden_layers=llama_config.num_hidden_layers)


    def init_fn_layerwise(rng):
        ...
        layer_states = [create_layer_state_from_params(params, idx, layer_name) for idx, layer_name in enumerate(layers)]
        return layer_states

    def create_trainstate_from_params(params):
        return TrainState.create(params=params, tx=all_optimizers[0], apply_fn=None) # pass optimizer just for init, not used
    ...
    def init_fn(rng):
        ...
        return TrainState.create(params=params, tx=all_optimizers[0], apply_fn=None)

    # def model_apply_layer(
    #     params: Dict[str, Any], # layer params
    #     *,
    ...
    # ):
    #     """
    #     Swap `layer_params` into `full_params` at `layer`, then run `model.apply`.
    #     """
    #     # 1. Merge layer into full param tree
    #     updated_params = merge_layer_params_partial(full_params, layer, params)

    #     # 2. Run model with updated parameters
    #     return model.apply(
    #         updated_params,                  # first positional arg: parameters
    #         batch_inputs,                    # second positional arg: inputs
    #         deterministic=deterministic,
    #         rngs=rngs,
    #     )
        

    def train_step_layer_jvp(layer_state, full_params0, rng, batch, wd):
        rng_generator = JaxRNG(rng)

        batch = with_sharding_constraint(batch, PS(('dp', 'fsdp')))
        layer = layer_state.layer

        layer_params0 = get_layer_params_partial(full_params0, layer)
        full_params_const0 = lax.stop_gradient(full_params0)

        def loss_and_accuracy(layer_params0, layer_params):
            dparams = jax.tree_util.tree_map(lambda x, y: x - y, layer_params, layer_params0)
            def f_batch(p):
                full_p = merge_layer_params_partial(full_params_const0, layer, p)
                logits = model.apply(
                    full_p, batch['input_tokens'], deterministic=False,
                    rngs=rng_generator(LLaMAConfigurator.rng_keys()),
                ).logits
                return logits
            primals, Jx = jax.jvp(f_batch, (layer_params0,), (dparams,))
            logits = primals + jax.lax.stop_gradient(Jx)
            full_p = merge_layer_params_partial(full_params_const0, layer, layer_state.layer_params)
            # logits = primals + Jx
            return cross_entropy_loss_and_accuracy_with_weight_decay(
                logits, batch['target_tokens'], full_p, full_params_const0, batch['loss_masks'], weight_decay=wd
            )
        grad_fn = jax.value_and_grad(loss_and_accuracy, has_aux=True)

        (loss, accuracy), grads = grad_fn(layer_params0, layer_state.layer_params)
        # print("grad norm", global_norm(grads))
        # Print which parameters have nonzero gradient
        # def find_nonzero_grads(grads, prefix=''):
        #     nonzero = []
        #     if isinstance(grads, dict):
        #         for k, v in grads.items():
        #             nonzero.extend(find_nonzero_grads(v, prefix + (str(k) + '/')))
        #     elif hasattr(grads, '_fields'):  # namedtuple, e.g., FrozenDict
        #         for k in grads._fields:
        #             v = getattr(grads, k)
        #             nonzero.extend(find_nonzero_grads(v, prefix + (str(k) + '/')))
        #     elif hasattr(grads, 'keys') and hasattr(grads, 'values'):
        #         for k in grads.keys():
        #             v = grads[k]
        #             nonzero.extend(find_nonzero_grads(v, prefix + (str(k) + '/')))
        #     elif hasattr(grads, 'shape'):  # assume array/tensor
        #         if getattr(grads, 'size', None) is not None and grads.size == 0:
        #             pass
        #         elif (grads != 0).any():
        #             nonzero.append(prefix[:-1])
        #     return nonzero

        # nonzero_grad_params = find_nonzero_grads(grads)
        # print("Params with nonzero gradients:", nonzero_grad_params)
        try:
            perplexity = jnp.exp(loss)
        except OverflowError:
            perplexity = jnp.float32("inf")
        layer_state = layer_state.apply_gradients(grads=grads) # layer state will handle merge back to full param structure
        metrics = dict(
            loss=loss,
            perplexity=perplexity,
            accuracy=accuracy,
            learning_rate=lr_sched(layer_state.step),
            gradient_norm=global_norm(grads),
            param_norm=global_norm(layer_state.layer_params),
            gpu_memory=get_gpu_memory()[0],
        )
        return layer_state, rng_generator(), metrics

    def train_step_gauss_newton(layer_state, full_params0, rng, batch, wd):
        rng_generator = JaxRNG(rng)
        batch = with_sharding_constraint(batch, PS(('dp', 'fsdp')))
        ...
        def value_and_gradient(layer_params0, layer_params):
            '''
            ...
            logits0, jvp_fn = linearize(f_batch, layer_params0)          # y0,   v = J(p0) dθ
            ...
            jt_fn = linear_transpose(jvp_fn, layer_params0) # primals just for shape/dtype
        ...
        (loss, accuracy), grads = value_and_gradient(layer_params0, layer_state.layer_params)
    ...
    example_params_shape = train_state_shapes.params

    # Bind layer 0 as a representative for partitioning
    creator0 = make_layer_state_creator(0, layers[0])
    layer_state_shapes = jax.eval_shape(creator0, example_params_shape)
    layer_state_partition = match_partition_rules(
        LLaMAConfigurator.get_partition_rules(), layer_state_shapes
    )
    ...
    # sharded_create_layer_state_from_params = pjit(
    #     create_layer_state_from_params,
    #     in_shardings=(layer_state_partition.params, None, None),
    #     out_shardings=layer_state_partition,
    #     donate_argnums=(0, ),
    # )
    ...
    #     in_shardings=(layer_state_partition, layer_state_partition.layer_params, PS(), batch_partition),
    #     out_shardings=(layer_state_partition, PS(), PS()),
    #     # donate_argnums=(0, 1),
    # )
    ...
    with mesh:
        ...
        # 1) Factory that closes over static args
        def make_layer_state_creator(layer_idx, layer_name):
            tx = all_optimizers[layer_idx]
            def creator(params):
                return LayerState.create(
                    full_params=params,
                    tx=tx,                  # static, differs per layer
                    apply_fn=model.apply,   # static
                    layer=layer_name,       # static, differs per layer
                    num_hidden_layers=llama_config.num_hidden_layers,
                )
            return creator

        # 2) For each layer, compute its OWN shapes & partitions (metadata matches!)
        example_params_shape = train_state_shapes.params
        per_layer_partitions = []
        per_layer_creators   = []
        for idx, lname in enumerate(layers):
            print('creating layer', lname)
            creator = make_layer_state_creator(idx, lname)
            layer_state_shapes_i = jax.eval_shape(creator, example_params_shape)
            layer_state_partition_i = match_partition_rules(
                LLaMAConfigurator.get_partition_rules(), layer_state_shapes_i
            )
            per_layer_creators.append(creator)
            per_layer_partitions.append(layer_state_partition_i)

        # 3) Compile one pjit per layer with matching out_shardings
        pjit_creators = [
            pjit(
                c,
                in_shardings=(train_state_partition.params,),
                out_shardings=lp,          # <- matches metadata for this layer
                # donate_argnums=(0,)
            )
            for c, lp in zip(per_layer_creators, per_layer_partitions)
        ]

        # 4) Create layer states (each pjit returns a LayerState whose metadata == its out_shardings)
        print('Creating layer states')
        layer_states = [pc(train_state.params) for pc in pjit_creators]
        ...
        for step in step_counter:
            ...
            print('step', step, 'train params start norm', global_norm(train_state.params))

            if FLAGS.reset_start:
                for l in range(len(layers)):
                    layer_params = get_layer_params_partial(train_state.params, layers[l])
                    layer_states[l] = layer_states[l].replace(
                        layer_params=layer_params, 
                        opt_state=all_optimizers[l].init(train_state.params)
                    )

            prefetched_batches = []
            for _ in range(FLAGS.inner_loop_iter):  # pre-fetch batches for the inner loop so each layer sees the same data
                try:
                    batch, dataset_metrics = next(dataset)
                    prefetched_batches.append((batch, dataset_metrics))
                except StopIteration:
                    print('Dataset exhausted')
                    break

            for l in range(len(layer_states)):
                # print('Training layer', layer_states[l].layer)
                ...
                for i in range(FLAGS.inner_loop_iter):
                    ...
                    old_layer_params = layer_states[l].layer_params
                    layer_states[l], sharded_rng, metrics = sharded_train_step_layer[l](
                        layer_states[l], train_state.params, sharded_rng, batch, FLAGS.inner_loop_wd
                    )
                    delta = jax.tree_util.tree_map(lambda x, y: x - y, layer_states[l].layer_params, old_layer_params)
            ...
            print('step', step, 'train params end norm', global_norm(train_state.params))
            print('step', step, 'new params norm', global_norm(merged_params))
            delta = jax.tree_util.tree_map(lambda x, y: x - y, merged_params, train_state.params)
            print("step", step, "delta norm", global_norm(delta))
            ...
            if FLAGS.linesearch:
                ...
                dir = jax.tree_util.tree_map(lambda x, y: x - y, merged_params, train_state.params)
                losses = []
                ...
                train_state = train_state.replace(params=jax.device_get(merged_params), step=step)
